...
test('metrics accept only bounded enumerated labels', () => {
  ...
  assert.throws(() => metrics.increment('unsafe', { tenant: 'ten_demo_alpha' }), /Unsafe metric label/);
  assert.throws(() => metrics.increment('unsafe', { message: 'arbitrary error message' }), /Unsafe metric label/);
});
