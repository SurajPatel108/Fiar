...
Phase 6 is implemented and locally verifiable. Controlled-pilot activation remains blocked until an operator selects and registers a real OIDC issuer/client, sets the final HTTPS dashboard origin and redirect URI, and installs independently generated mounted secrets. The deterministic PostgreSQL fake provider remains the only execution provider and cannot move real money.
...
Managers and admins use OIDC Authorization Code Flow with PKCE S256. Discovery, HTTPS endpoints in production, issuer, audience, signature, algorithm, expiry, nonce, state, and PKCE are validated. `(issuer, subject)` must already map to an active manager/admin principal; first-login privilege creation is forbidden. No username/password database or fake production login exists.
...
Metrics require a separate bearer secret and are internal in Compose. Labels are fixed enums only; tenant/principal/action/order IDs, hashes, comments, credentials, and arbitrary errors are never labels. Metrics cover decisions, authentication failures, approval outcomes, worker claims/retries, execution/reconciliation outcomes, kill-switch blocks, queue depth/age, pending approvals, and pending reconciliation count/age. Counters and authentication throttling are process-local, appropriate only for the single-instance controlled pilot.
